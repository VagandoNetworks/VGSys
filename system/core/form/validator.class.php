...
class Core_Form_Validator {
    ...
    {
        if (count(Core::getLib('request')->getRequest()) == 0)
        {
            return $this;
        }
    ...
    public function validate()
    {
        ...
        foreach ($this->_fieldData as $field => $row)
        {
            ...
            $this->_fieldData[$field]['value'] = Core::getLib('request')->get($field);
	...
	{
		$field = Core::getLib('request')->get($field);
    ...
    {
        return Core::getLib('parse.input')->cleanTitle($str);
    }
    ...
    {
        return Core::getLib('parse.input')->prepare($str);
    }
}