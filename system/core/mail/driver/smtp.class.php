...
class Core_Mail_Driver_Smtp {
    ...
    public function __construct()
    {
        ...
        if (Core::getParam('core.mail_smtp_auth'))
        {
            $this->_mail->SMTPAuth = true;
            $this->_mail->Username = Core::getParam('core.mail_smtp_user');
            $this->_mail->Password = Core::getParam('core.mail_smtp_pass');
        }
        $this->_mail->Port = Core::getParam('core.mail_smtp_port');
        $this->_mail->Host = Core::getParam('core.mail_smtp_host');
    ...
    public function send($to, $subject, $textPlain, $textHtml, $fromName = null, $fromEmail = null)
    {
        ...
        if ( ! $this->_mail->Send())
        {
            ...
            return Core_Error::trigger($this->_mail->ErrorInfo);
        }
